...
function Login({ server, onLogin, onBack }) {
  ...
  const submit = () => {
    if (showPlatform) { const d = demo.platform; if (u === d.user && p === d.pass) onLogin({ role: "platform", entityId: d.entity }); else setErr("بيانات الدخول غير صحيحة."); return; }
    if (effectiveRole === "company") { const d = companyRole === "ops" ? demo.companyOps : demo.company; if (u === d.user && p === d.pass) onLogin({ role: "company", entityId: d.entity, companyRole }); else setErr("بيانات الدخول غير صحيحة."); return; }
    const d = demo[effectiveRole];
    if (u === d.user && p === d.pass) onLogin({ role: effectiveRole, entityId: d.entity });
      ...
      <div className="flex items-center justify-center p-6" style={{ background: C.bg }}>
        <div className="w-full max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <PrimaryBtn full icon={ArrowRight} onClick={submit}>دخول</PrimaryBtn>
    ...
    {tasks.length ? tasks.map((t) => { const m = data.merchants.find((x) => x.id === t.merchantId);
      ...
      return (<Card key={t.id} className="p-4 sm:p-5 min-w-0">
        ...
        {!t.gpsConfirmed ? <div className="rounded-xl p-3 mb-3" style={{ background: C.infoSoft + "55", border: `1px solid ${C.info}` }}>
          ...
          <LocationCard m={m} />
          <div className="mt-2"><PrimaryBtn full icon={Navigation} onClick={() => { actions.pmConfirmGps(t.id); toast("تم التحقق — أنت داخل محيط المحل ✓"); }}>أنا الآن في محيط المحل — تأكيد GPS</PrimaryBtn></div>
        </div> : <>
        ...
        {req.photoBefore && <div className="rounded-xl p-3 mb-2.5 flex items-center justify-between gap-2 flex-wrap" style={{ background: C.bg }}>
          ...
          {t.photoBefore ? <span className="text-xs font-bold flex items-center gap-1" style={{ color: C.success }}><CheckCircle2 size={14} />التُقطت 📸</span> : <GhostBtn sm icon={ImageIcon} onClick={() => { actions.pmPhoto(t.id, "photoBefore"); toast("التُقطت صورة قبل 📸"); }}>التقاط</GhostBtn>}
        </div>}
        ...
        {req.photoAfter && <div className="rounded-xl p-3 mb-2.5 flex items-center justify-between gap-2 flex-wrap" style={{ background: C.bg }}>
          ...
          {t.photoAfter ? <span className="text-xs font-bold flex items-center gap-1" style={{ color: C.success }}><CheckCircle2 size={14} />التُقطت 📸</span> : <GhostBtn sm icon={ImageIcon} onClick={() => { actions.pmPhoto(t.id, "photoAfter"); toast("التُقطت صورة بعد 📸"); }}>التقاط</GhostBtn>}
        </div>}
  ...
  return (<div className="space-y-4"><SectionTitle sub="حصر أدوات المحل، عقود الثلاجات، وطلب الجرد ومطابقة الحساب من التاجر.">حصر أدوات المحل والجرد والمطابقة</SectionTitle>
    ...
    <Card className="p-4 min-w-0"><h3 className="font-extrabold mb-1 flex items-center gap-2" style={{ color: C.text }}><FileSignature size={17} style={{ color: SC.color }} />تخزين عقد الثلاجة</h3>
      ...
      <div className="flex gap-2 mt-3">
        <GhostBtn icon={ImageIcon} onClick={() => { setFc({ ...fc, photo: true }); toast("التُقطت صورة العقد 📸"); }}>{fc.photo ? "صورة العقد ✓" : "تصوير العقد"}</GhostBtn>
        <PrimaryBtn icon={CheckCircle2} onClick={() => { if (!fc.no) { toast("أدخل رقم العقد", "danger"); return; } actions.saveFridgeContract(fc.merchantId, { no: fc.no, model: fc.model, date: fc.date, note: fc.note, photo: !!fc.photo }); toast("حُفظ عقد الثلاجة ✓"); setFc({ ...fc, no: "", model: "", date: "", note: "", photo: false }); }}>حفظ العقد</PrimaryBtn>
      </div>
    </Card>
        ...
        {isOpen && <div className="p-3.5 space-y-3" style={{ borderTop: `1px solid ${C.line}`, background: C.bg }}>
          {!fieldDone ? <>
            ...
            <div className="rounded-xl p-3 flex items-center justify-between gap-2" style={{ background: C.surface }}>
              ...
              {ff.photoed ? <span className="text-xs font-bold flex items-center gap-1" style={{ color: C.success }}><CheckCircle2 size={14} />صورة ملتقطة 📸</span> : <GhostBtn sm icon={ImageIcon} onClick={() => { setP(p.id, { photoed: true }); toast("التُقطت صورة المحل 📸"); }}>التقاط صورة</GhostBtn>}
            </div>
            {/* 3: GPS داخل النطاق */}
            <div className="rounded-xl p-3" style={{ background: C.surface }}>
              ...
              {ff.atStore ? <div className="text-xs rounded-lg p-2 flex items-center gap-1.5" style={{ background: C.successSoft, color: C.success }}><MapPinned size={14} />تم التحقق: أنت داخل نطاق المحل — سُجّلت الإحداثيات.</div>
                : <PrimaryBtn sm full icon={Navigation} onClick={() => { setP(p.id, { atStore: true }); toast("وُثّق الموقع — أنت داخل النطاق ✓"); }}>أنا الآن أمام المحل — وثّق موقعي</PrimaryBtn>}
            </div>
            <PrimaryBtn full tone="accent" icon={CheckCircle2} onClick={() => { if (!ff.photoed || !ff.atStore) { toast("أكمل الصورة وتوثيق الموقع أولاً", "danger"); return; } actions.repVerifyClient(p.id, { correctedName: ff.name, photo: "📸", lat: 32.885 + Math.random() * 0.02, lng: 13.18 + Math.random() * 0.03 }); toast("وُثّق المحل — اطلب الآن تأكيد التاجر ✓"); }}>إتمام التوثيق الميداني</PrimaryBtn>
          </> : <>
            {/* 4: تأكيد التاجر */}
            <div className="rounded-xl p-3" style={{ background: C.surface }}>
...
export default function App() {
  ...
  const actions = {
    ...
    pmConfirmGps: (id) => upd((d) => { const t = d.promotions.find((x) => x.id === id); const m = d.merchants.find((x) => x.id === t?.merchantId); return { ...d, promotions: d.promotions.map((x) => x.id === id ? { ...x, gpsConfirmed: true, status: "arrived", lat: m?.lat, lng: m?.lng } : x) }; }),
    pmPhoto: (id, which) => upd((d) => ({ ...d, promotions: d.promotions.map((x) => x.id === id ? { ...x, [which]: true } : x) })),
  ...
  return (<>{hosts}<Shell server={server} user={user} data={data} actions={actions} onLogout={() => setUser(null)} /></>);
}